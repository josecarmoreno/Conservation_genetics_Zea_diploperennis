...
## We use a projection of 86 because we have 43 diploid individuals
## We indicate to create 100 bootraps (Nboot) using a chunk size of 1000000

vcf_file = '/home/jose-carlos-moreno-juarez/Documentos/Z_diploperennis/data/Filtering/HS_demography/dataset5/diploperennis_dataJa_d5.vcf.gz'
popfile = '/home/jose-carlos-moreno-juarez/Documentos/Z_diploperennis/data/Filtering/HS_demography/dataset5/input_Ja.txt'
pop_ids = ['Jalisco']
projections = [86] 
Nboot = 100
chunk_size = 1e6
output_dir = '/home/jose-carlos-moreno-juarez/Documentos/Z_diploperennis/out/dadi/SFS/Jalisco_boot'
...
try:
    ...
    fs_path = os.path.join(output_dir, 'Jalisco.fs')
    fs_original.to_file(fs_path)
    print("✅ SFS original generado: Jalisco.fs")
    ...
    plt.title("SFS - Jalisco")
    png_path = os.path.join(output_dir, 'Jalisco.png')
    plt.savefig(png_path, dpi=300)
    plt.close()
    print("🖼️ Imagen del SFS guardada: Jalisco.png")
...
for i in range(Nboot):
    ...
    try:
        ...
        if not fs.folded:
            fs = fs.fold()
        fs.to_file(os.path.join(output_dir, f'Jalisco.boot_{i}.fs'))
        success += 1
    except Exception as e:
